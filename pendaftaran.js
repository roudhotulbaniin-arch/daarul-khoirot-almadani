<script type="module">
    ...
    const firebaseConfig = {
        ...
        appId: "1:963910462284:web:17de15dc5d2481b20b3de9"
    };

    const app = initializeApp(firebaseConfig);
    const db = getFirestore(app);

    // Handler Utama Submit Form Pendaftaran
    async function handleFormSubmit(event) {
        ...
        try {
            ...
            formData.forEach((value, key) => {
                // Abaikan file dan abaikan penanganan riwayat_sakit bawaan formData agar tidak bentrok
            ...
            // 🔥 FIX RADIKAL UNTUK FIREBASE: Ambil Checkbox Riwayat Sakit
            ...
            // Evaluasi kondisi kesehatan santri
            if (dataFinal['w_sehat'] === "Sehat" || arraySakit.length === 0) {
                // Firebase terkadang menolak array kosong [], kita isi dengan string penanda agar field-nya wajib muncul di Firestore
...
/* =========================================================
   SINKRONISASI DATA WILAYAH & KODE POS (ULTRA COMPATIBLE)
========================================================= */
...
function loadKabIbu(val) { if(val && val !== "provinces_init_val") loadWilayah(`regencies/${val}`, 'kab_ibu', 'Pilih Kabupaten'); }
function loadKecIbu(val) { if(val) loadWilayah(`districts/${val}`, 'kec_ibu', 'Pilih Kecamatan'); }
...
function loadKabSantri(val) { if(val && val !== "provinces_init_val") loadWilayah(`regencies/${val}`, 'kab_santri', 'Pilih Kabupaten'); }
function loadKecSantri(val) { if(val) loadWilayah(`districts/${val}`, 'kec_santri', 'Pilih Kecamatan'); }
...
/* =========================================================
   LOGIKA REFORMASI LOCK FIELDS & REPLIKASI DOMISILI (FULL NAME MODE)
========================================================= */
...
function isiAlamatPesantren() {
    ...
    const setSelectByName = (name, val, text) => {
    ...
    setSelectByName('prov_santri', dataPesantren.prov, dataPesantren.prov_text);
    setSelectByName('kab_santri', dataPesantren.kab, dataPesantren.kab_text);
    setSelectByName('kec_santri', dataPesantren.kec, dataPesantren.kec_text);
    setSelectByName('desa_santri', dataPesantren.desa, dataPesantren.desa_text);

    if (getElByName('al_santri')) getElByName('al_santri').value = dataPesantren.al;
    if (getElByName('rt_santri')) getElByName('rt_santri').value = dataPesantren.rt;
    if (getElByName('rw_santri')) getElByName('rw_santri').value = dataPesantren.rw;
    if (getElByName('pos_santri')) getElByName('pos_santri').value = dataPesantren.pos;
}
...
function toggleDomisiliIbu(val) {
    ...
    if (box) {
        box.style.display = (val === "") ? 'none' : 'grid';
    }
...
/* =========================================================
   FIXED: LOGIKA PENGUNCIAN FIELD STATUS AYAH (READONLY & DISABLED)
========================================================= */
...
/* =========================================================
   FUNGSI NAVIGASI & VALIDASI TERPADU
========================================================= */
function validateInput(tabId) {
    ...
    // 1. Cek apakah kosong
    // 2. Cek apakah masih nilai default "-- Pilih"
    ...
    if (tabId === 'info') {
        return true; // Tab Info tidak memiliki validasi form inputan khusus
    }
...
function handleNoNISN(checked) {
    const nisnInput = document.getElementsByName('nisn')[0]; 
    if (!nisnInput) return;
    
    if (checked) {
        nisnInput.value = '0000000000'; 
        nisnInput.readOnly = true;
        nisnInput.style.backgroundColor = '#e9ecef';
        nisnInput.style.color = '#6c757d';
        nisnInput.style.cursor = 'not-allowed';
    } else {
        nisnInput.value = '';
        nisnInput.readOnly = false;
        nisnInput.style.backgroundColor = '#ffffff';
        nisnInput.style.color = '#000000';
        nisnInput.style.cursor = 'text';
    }
}

function toggleHP(cb, inputName) {
    const hpInput = document.getElementsByName(inputName)[0];
    if (!hpInput) return;
    
    if (cb.checked) {
        hpInput.value = 'TIDAK MEMILIKI';
        hpInput.readOnly = true;
        hpInput.style.backgroundColor = '#e9ecef';
        hpInput.style.color = '#6c757d';
        hpInput.style.cursor = 'not-allowed';
    } else {
        hpInput.value = '';
        hpInput.readOnly = false;
        hpInput.style.backgroundColor = '#ffffff';
        hpInput.style.color = '#000000';
        hpInput.style.cursor = 'text';
    }
}
...
/* =========================================================
   INTEGRASI WHATSAPP REPORT NOTIFICATION (FIXED)
========================================================= */
function kirimWA(data) {
    const gV = (name) => {
        if (data) {
            if (name === 'nama' || name === 'nama_santri') {
                return (data['nama_santri'] || data['nama'] || "-").trim();
            }
            if (name === 'no_kk' || name === 'kk_santri') {
                return (data['no_kk'] || data['kk_santri'] || "-").trim();
            }
            if (name === 'jenis_kelamin' || name === 'jk_santri') {
                return (data['jenis_kelamin'] || data['jk_santri'] || "-").trim();
            }
            if (name === 'cita' || name === 'cita_cita') {
                return (data['cita'] || data['cita_cita'] || "-").trim();
            }
            if (name === 'hobi' || name === 'hobi_santri') {
                return (data['hobi'] || data['hobi_santri'] || "-").trim();
            }
    ...
    // 🔥 FIX RADIKAL RIWAYAT KESEHATAN UNTUK WHATSAPP
...
/* =========================================================
   SISTEM NAVIGASI DINAMIS DI LUAR KARTU FORM
========================================================= */
...
function perbaruiTombolNavigasi(currentId) {
    ...
    // Tombol Kembali: Sembunyikan di tab awal, muncul di tab selanjutnya
    btnPrev.style.display = (currentId === 'info' || currentId === 'santri') ? 'none' : 'flex';

    // Tombol Utama: Ubah label saat di tab terakhir
    if (currentId === 'pernyataan') {
        btnMain.innerHTML = '<i class="fas fa-save fa-lg"></i> Simpan';
        btnMain.className = 'btn-simpan-final'; // CSS khusus simpan (misal warna hijau)
    } else {
        btnMain.innerHTML = 'Lanjut <i class="fas fa-chevron-circle-right fa-lg"></i>';
        btnMain.className = 'btn-next'; // CSS standar
    }
}

// Menentukan aksi tombol utama
function handleMainAction() {
    const currentId = dapatkanTabAktif();
    
    if (currentId === 'pernyataan') {
        // Jalankan proses simpan dan kirim WA di sini
        prosesSimpanFinal(); 
    } else {
        // Jalankan navigasi seperti biasa
        navigasiMaju();
    }
}
...
function navigasiMaju() {
    ...
    if (validateInput(currentId)) {
        if (currentIndex < urutanTab.length - 1) {
            const nextId = urutanTab[currentIndex + 1];
            openTab(null, nextId);
        }
    }
}
...
function navigasiMundur() {
    ...
    if (currentIndex > 0) {
        const prevId = urutanTab[currentIndex - 1];
        openTab(null, prevId);
    }
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function toggleRiwayat() {
        var select = document.getElementById("selectKesehatan");
        var divRiwayat = document.getElementById("divRiwayatSakit");
        
        if (select.value === "Pernah Sakit") {
