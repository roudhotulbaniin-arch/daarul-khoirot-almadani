...
function isiOpsiJabatan() {
    ...
    const currentValue = filterJabatan.value;
    
    // 1. Kumpulkan jabatan unik dari data user
    const jabatanSet = new Set();
    daftarUser.forEach(u => {
        if (u.jabatan?.trim()) jabatanSet.add(u.jabatan.trim());
    });
    
    // 2. Tambahkan jabatan default (biar selalu ada meski belum ada usernya)
    ...
    filterJabatan.setAttribute('data-cd-icons', JSON.stringify(iconMap));
    ...
    jabatanList.forEach(j => {
        ...
        opt.value = j;
        opt.textContent = j;
        filterJabatan.appendChild(opt);
    });
    
    // 5. Restore value
    if (currentValue) filterJabatan.value = currentValue;
    
    // 6. ⭐ REFRESH CUSTOM DROPDOWN
...
/* ================================================================
   🎯 FIX MENU DROPDOWN — Positioning & Z-Index
================================================================ */

/* WRAPPER — Pastikan menu keluar dengan benar */
.cd-wrapper {
    position: relative !important;
    z-index: 1;  /* Base z-index */
}

/* Saat menu terbuka, wrapper harus di atas elemen lain */
.cd-wrapper.open {
    z-index: 9999 !important;
}

/* MENU DROPDOWN — Posisi selalu di bawah trigger (default) */
.cd-menu {
    ...
    right: 0 !important;
    width: 100% !important;
    min-width: 200px !important;  /* Minimum lebar */
    ...
    z-index: 10000 !important;  /* ⭐ Tinggi banget biar di atas semua */
    display: none;
    
    box-sizing: border-box;
}

/* Kalau harus open-up (menu ke atas) */
.cd-wrapper.open-up .cd-menu {
    top: auto !important;
    bottom: calc(100% + 6px) !important;
}

/* Saat menu terbuka */
.cd-wrapper.open .cd-menu {
    display: flex !important;
...
/* ================================================================
   🎯 FIX FILTER BAR — Overflow untuk Dropdown
================================================================ */

/* KARTU MODERN — JANGAN overflow: hidden kalau ada dropdown! */
.kartu-modern-clean {
    overflow: visible !important;  /* ⭐ WAJIB agar dropdown bisa keluar */
}

/* Table wrapper masih boleh overflow hidden (untuk scroll horizontal) */
.table-wrapper {
...
.filter-bar .cd-wrapper {
    position: relative !important;
}


/* ================================================================
   📱 MOBILE — Menu Dropdown Full Width & Positioned Right
================================================================ */

@media (max-width: 768px) {
    /* Menu dropdown di mobile — full width dalam wrapper */
    .filter-bar .cd-menu {
    ...
    /* Kalau menu untuk dropdown kedua (Semua Jabatan), 
       ke arah kiri biar tidak overflow */
    .filter-bar .filter-group .cd-wrapper:nth-child(2) .cd-menu {
        left: auto !important;
        right: 0 !important;
    }
}
...
@media (max-width: 480px) {
    /* Menu di HP kecil — sesuaikan lebar minimum */
    .filter-bar .cd-menu {
